...
// Shared cookie options so login/logout always agree on how the
// auth cookie was set (mismatched options mean logout silently fails
...
export const getMe = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (role === 'admin') {
      ...
      account = await Agent.findById(id).select('-password');
    } else {
    ...
    const userResponse = {
      ...
      userPhone: account.userPhone,
      userRole: role,
      status: (account as { status?: string }).status ?? 'active',
      balance: account.balance,
      photo: (account as { userPhoto?: string }).userPhoto,
    };
