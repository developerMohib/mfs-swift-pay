...
const SendMoney = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Basic validation (optional, add more if needed)
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `You are about to send ৳${amount} to ${recipientPhone}`,
    ...
    // If user cancelled or transaction failed in preConfirm
    if (!result.isConfirmed || !result.value) {
      return;
    }

    // Success! Show nice confirmation
    Swal.fire({
      title: "Money Sent Successfully!",
      html: `You have successfully sent <strong>৳${amount}</strong> to <strong>${recipientPhone}</strong>`,
      icon: "success",
      timer: 3000,
